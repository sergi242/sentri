                ...
                <div class="col-xl-3 col-md-6 col-12">
                    <div class="card">
                        <div class="text-center">
                            <div class="card-body">
                                <h4 class="card-title">
                                    <strong>{{ $demande->nomcomplet() }}</strong>
                                </h4>
                                <img src="{{ asset("app/".$demande->demandes->last()->photo) }}" class="rounded-circle  height-150" alt="Card image">
                            </div>
                            <div class="card-body">
                                <h5 class="text-muted">
                                    {{ date("d/m/Y",strtotime($demande->date_naissance)) }} - {{ $demande->sexe == "Masculin" ? "Homme":"Femme" }}
                                </h5>
                                <h6 class="card-subtitle text-muted">{{ $demande->pays?->lib_pays }} <br> ( {{ $demande->demandes->last()->type_demande }} )</h6>
                            </div>
                            <div class="text-center">
                                {{-- <a href="#" class="btn btn-social-icon mr-1 mb-1 btn-outline-facebook"><span class="la la-facebook"></span></a> --}}
                                <h6 class="text-muted"><strong>Depuis {{ TechnoDev::timespan($demande->demandes->last()->created_at) }}</strong></h6>
                                <a href="{{ route("demandes.show",$demande->demandes->last()->id) }}" class="btn btn-secondary btn-sm mb-1">Voir détail</a>
