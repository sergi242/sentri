...
<!-- END: Page Vendor JS-->
<script src="{{asset("res/app-assets/js/scripts/charts/chartjs/line/line.js")}}"></script>
<script>
    $(window).on("load", function(){
        var canvas = document.getElementById("line-chart");
        var route = "{{route("graphes.demandes")}}";
        $("form.search").on("submit",function(){
            var month = $("#month").val();
            var year = $("#year").val();
            load(canvas,month,year,route);
            return false;
        });
    ...
    function load(div,month,year,route){
        $.get(route,{month:month,year:year},function(response){
                ...
                if(month && year){
                    ...
                    var approuvees = response.approuvees;
                    var theRemovedTotal = totals.shift();
                    var moisRemoved = approuvees.shift();
                    var matrix = [
                        totals,
                        approuvees
                    ];
