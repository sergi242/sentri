...
class DemandeController extends Controller
{
    ...
    {
        $demandes = Demande::where("statut_demande","Approuvée")->orderBy("updated_at","desc")->paginate(20);
    ...
    public function attente_attribution(){
        $demandes = Demande::where("statut_demande","Approuvée")->where("attribue","0")->orderBy("updated_at","desc")->paginate(20);
        $layout = request()->get("layout");
        if($layout != null){
            request()->session()->put("layout",$layout);
        }
    ...
    public function attribuees(){
        $demandes = Demande::where("attribue","1")->orderBy("updated_at","desc")->paginate(20);
        $layout = request()->get("layout");
        if($layout != null){
            request()->session()->put("layout",$layout);
        }
    ...
    {
        $demandes = Demande::where("statut_demande","En attente d'approbation")->orderBy("updated_at","desc")->paginate();
        $layout = request()->get("layout");
        if($layout != null){
            request()->session()->put("layout",$layout);
        }
    ...
    public function show(string $id)
    {
        ...
        $fiche_validite = $demande->date_validiter_fiche;
        $expiration_fiche = Carbon::parse($fiche_validite)->addDays(30);

        $sims = collect($similaires);
        //toastr()->warning("Cet impétrant présente ".$sims->count() ." similarité(s)");
        return view("admin.demandes.show",compact("demande","pays","departements","etatsCivils","validites","sims","categories","fiche_validite"));
    }
