    ...
    public function index(Request $request)
{
    ...
    if ($request->filled('nb_demandes')) {
        match($request->nb_demandes) {
            '0'  => $query->having('demandes_count', '=', 0),
    ...
    public function rejectSimilarity($demandeId, $similaireId)
    {
        ...
        IdentitySimilarityService::compare($demande->impetrant, $similaire->impetrant);
    ...
    public function approuvees(Request $request)
    {
        ...
        $demandes = $query->orderBy("updated_at", "desc")->paginate(20)->withQueryString();
        $layout = $request->get("layout");
        if ($layout != null) { $request->session()->put("layout", $layout); }

        $status = "La liste des toutes les demandes approuvées";
        $pays = Pays::orderBy('lib_pays')->get();
        $typesDemandes = Demande::distinct()->orderBy('type_demande')->pluck('type_demande');

        if (request()->session()->get("layout") == "cards") {
            return view("admin.demandes.cards_list", compact("demandes", "status", "pays", "typesDemandes"));
        } else {
            return view("admin.demandes.approvee", compact("demandes", "status", "pays", "typesDemandes"));
        }
    }

    public function impressioncartes()
    {
        $demandes = Demande::where("tag_demande","IMPRESSION")
            ->where("statut_demande","Approuvée")
            ->where("attribue","0")
            ->where("type_demande","Carte de résident temporaire")
            ->paginate(20);
        ...
        if(request()->session()->get("layout") == "cards"){
            ...
            return view("admin.demandes.impressioncarte", compact("demandes", "status", "pays", "typesDemandes"));
        }
    }
    ...
    public function attente_attribution()
    {
        ...
        if(request()->session()->get("layout") == "cards"){
            ...
            return view("admin.demandes.pendingattribution", compact("demandes", "status", "pays", "typesDemandes"));
        }
    }
    ...
    public function attribuees()
    {
        ...
        if(request()->session()->get("layout") == "cards"){
            ...
            return view("admin.demandes.attribuees", compact("demandes", "status", "pays", "typesDemandes"));
        }
    }

    public function contentieux(Request $request)
    {
        ...
        if(request()->session()->get("layout") == "cards"){
            ...
            return view("admin.demandes.contentieux", compact("demandes", "status", "pays", "typesDemandes"));
        }
    }

    public function attentes(Request $request)
    {
        $query = Demande::with('impetrant')->where("statut_demande", "En attente d'approbation")
                        ->where("updated_at", ">=", Carbon::now()->subDays(30));
        ...
        if(request()->session()->get("layout") == "cards"){
            ...
            return view("admin.demandes.attentes", compact("demandes", "status", "pays", "typesDemandes"));
        }
    }

    public function resetDocument($id)
    {
        $demande = Demande::findOrFail($id);
        $demande->numero_document  = null;
        $demande->date_emission    = null;
        $demande->date_expiration  = null;
        $demande->date_attribution = null;
        $demande->attribue         = 0;
        $demande->attribue_par     = null;
        $demande->save();

        toastr()->success("Document réinitialisé avec succès");
...
public function create(Request $request)
{
    ...
    $validites    = ["1","3","5"];
    $usersActifs  = \App\Models\User::where('active', 1)->orderBy('nom')->get(['id','nom','prenom','grades_id']);

    // Pré-remplissage depuis un impétrant existant
    $impetrantPrefill = null;
    if ($request->filled('impetrant_id')) {
        $impetrantPrefill = \App\Models\Impetrant::find($request->input('impetrant_id'));
    }

    return view("admin.demandes.create", compact(
        "pays", "departements", "etatsCivils", "validites", "usersActifs", "impetrantPrefill"
    ));
}
    ...
    public function newvisa()
    {
        ...
        return view("admin.demandes.newvisa",compact("pays","departements","etatsCivils","validites","pieces","categories","employeurs","usersActifs"));
    }
   ...
   public function newcrt()
{
    ...
    $categories = CategorieSocioProfessionnelle::all();
    $usersActifs = \App\Models\User::where('active', 1)->orderBy('nom')->get(['id','nom','prenom','grades_id']);
    return view("admin.demandes.newcrt",compact("pays","departements","etatsCivils","validites","pieces","categories","employeurs","usersActifs"));
}
    ...
    public function searchdocument(Request $request){
        ...
        if($type == "PASSEPORT"){
            ...
            if($results->count() <= 0){ toastr()->warning("Aucune donnée liée à cette recherche"); }
            return view("admin.demandes.renewsearch",compact("results"));
    ...
    // =========================================================================
    // RENOUVELER — Affiche le formulaire
    // Bloque si dernière demande < 6 mois, dérogation possible pour Admin
    // =========================================================================
    public function renouveler($id)
    {
        ...
        // ── Chercher la dernière demande de cet impétrant ─────────────────────
        ...
        if ($derniereDemande) {
            $dateRef       = $derniereDemande->date_emission ?? $derniereDemande->date_demande;
            $dateEmission  = Carbon::parse($dateRef);
            $maintenant    = Carbon::now('Africa/Brazzaville');
            $joursEcoules  = (int) $dateEmission->diffInDays($maintenant);
            $delaiMinJours = 183;

            if ($joursEcoules < $delaiMinJours) {
                $joursRestants      = $delaiMinJours - $joursEcoules;
                $prochaineDateDispo = $dateEmission->copy()->addDays($delaiMinJours)->format('d/m/Y');
                $roleActuel         = Auth::user()?->role?->lib_role;
                $peutDeroger        = in_array($roleActuel, ['SuperAdmin', 'Admin']);

                if (!$peutDeroger) {
                    // ── Agent normal : blocage total ──────────────────────────
                    try {
                        $user = Auth::user();
                        DB::table('audit_logs')->insert([
                            'user_id'     => Auth::id(),
                            'user_name'   => $user?->prenom . ' ' . $user?->nom,
                            'user_role'   => $roleActuel ?? '',
                            'action'      => 'RENOUVELLEMENT_BLOQUE',
                            'module'      => 'demandes',
                            'entity_id'   => $derniereDemande->id,
                            'entity_type' => 'Demande',
                            'old_values'  => null,
                            'new_values'  => null,
                            'status'      => 'forbidden',
                            ...
                            'city'        => null,
                            'description' => sprintf(
                                'Renouvellement bloqué — %s %s — dernière demande le %s (%d j écoulés). Restant : %d j. Dispo le : %s.',
                                $impetrant->nom, $impetrant->prenom,
                                $dateEmission->format('d/m/Y'),
                                $joursEcoules, $joursRestants, $prochaineDateDispo
                            ),
                            'created_at'  => DB::raw("CONVERT_TZ(NOW(), '+00:00', '+01:00')"),
                            'updated_at'  => DB::raw("CONVERT_TZ(NOW(), '+00:00', '+01:00')"),
                        ]);
                    } catch (\Exception $e) {
                        Log::channel('loggin')->error('audit_logs insert failed: ' . $e->getMessage());
                    }

                    toastr()->error(
                        "Renouvellement impossible. Dernière demande le {$dateEmission->format('d/m/Y')} "
                        . "({$joursEcoules} j / {$delaiMinJours} requis). "
                        . "Prochain renouvellement autorisé le : {$prochaineDateDispo}.",
                        "Délai non atteint"
                    );

                    return redirect()->route('demandes.renouvellement');
                }

                // ── Admin/SuperAdmin : formulaire avec flag dérogation ────────
                toastr()->warning(
                    "Délai non atteint ({$joursEcoules} j / {$delaiMinJours} requis). "
                    . "En tant qu'administrateur, vous pouvez accorder une dérogation.",
                    "Dérogation requise"
                );
                ...
                $categories   = CategorieSocioProfessionnelle::all();
                $employeurs   = Employeur::all();
                $usersActifs  = \App\Models\User::where('active', 1)->orderBy('nom')->get();

                return view(
                    "admin.demandes.renouvellement",
                    compact("impetrant","pays","departements","etatsCivils","validites","categories","employeurs","usersActifs")
                )->with([
                    'derogation_requise' => true,
                    'jours_ecoules'      => $joursEcoules,
                    'jours_restants'     => $joursRestants,
                    'prochain_dispo'     => $prochaineDateDispo,
                    'date_derniere'      => $dateEmission->format('d/m/Y'),
                ]);
            }
        }

        // ── Délai OK (ou première demande) : afficher le formulaire ──────────
        $pays         = Pays::all();
        $departements = Departement::all();
        $etatsCivils  = ['Célibataire','Marié(e)','Divorcé(e)','Veuf(-ve)'];
        $validites    = ["1","3","5"];
    ...
    // =========================================================================
    // RENEWSTORE — Enregistre la demande de renouvellement
    // Double vérification serveur + gestion dérogation Admin
    // =========================================================================
    public function renewstore(Request $request, $id)
    {
        $request->validate([
            ...
            "telephone"                         => "required",
            "numero_passeport"                  => $request->sans_passeport ? "nullable" : "required",
            "date_emission_passeport"           => $request->sans_passeport ? "nullable" : "required",
            "date_expiration_passeport"         => $request->sans_passeport ? "nullable" : "required",
            "passeport_delivre_par"             => $request->sans_passeport ? "nullable" : "required",
        ...
        if ($derniereDemande) {
            ...
            if ($joursEcoules < $delaiMinJours) {
                ...
                if ($peutDeroger) {
                    $motif = $request->filled('motif_derogation')
                        ? $request->motif_derogation
                        : '(aucun motif saisi — dérogation accordée par ' . Auth::user()?->prenom . ' ' . Auth::user()?->nom . ')';
                                            // ── Dérogation accordée par Admin ─────────────────────────
                         try {
                        $user = Auth::user();
                        DB::table('audit_logs')->insert([
                            'user_id'     => Auth::id(),
                            'user_name'   => $user?->prenom . ' ' . $user?->nom,
                            'user_role'   => $roleActuel ?? '',
                            'action'      => 'DEROGATION_RENOUVELLEMENT',
                            'module'      => 'demandes',
                            'entity_id'   => $derniereDemande->id,
                            'entity_type' => 'Demande',
                            'old_values'  => null,
                            ...
                            'city'        => null,
                         'description' => sprintf(
                            'DÉROGATION accordée par %s — Impétrant : %s %s — Délai : %d j / %d requis. Motif : "%s".',
                            $user?->prenom . ' ' . $user?->nom,
                            $impetrant->nom, $impetrant->prenom,
                            $joursEcoules, $delaiMinJours,
                            $motif   // ← utiliser la variable avec fallback
                        ),
                            'created_at'  => DB::raw("CONVERT_TZ(NOW(), '+00:00', '+01:00')"),
                            'updated_at'  => DB::raw("CONVERT_TZ(NOW(), '+00:00', '+01:00')"),
                        ]);
                    } catch (\Exception $e) {
                        Log::channel('loggin')->error('audit_logs derogation: ' . $e->getMessage());
                    }

                    toastr()->warning(
                        "Dérogation accordée. Renouvellement enregistré malgré le délai non atteint.",
                        "Dérogation — " . Auth::user()?->prenom
                    );
                    // Le code continue vers l'enregistrement

                } else {
                    // ── Blocage : agent normal ou Admin sans motif ────────────
                    try {
                        $user = Auth::user();
                        DB::table('audit_logs')->insert([
                            'user_id'     => Auth::id(),
                            ...
                            'city'        => null,
                            'description' => sprintf(
                                '[POST] Renouvellement bloqué — %s %s — dernière demande le %s (%d j). Restant : %d j.',
                                $impetrant->nom, $impetrant->prenom,
                                $dateEmission->format('d/m/Y'),
                                $joursEcoules, $joursRestants
                            ),
                            'created_at'  => DB::raw("CONVERT_TZ(NOW(), '+00:00', '+01:00')"),
                            'updated_at'  => DB::raw("CONVERT_TZ(NOW(), '+00:00', '+01:00')"),
                        ]);
                    ...
                    toastr()->error(
                        "Renouvellement refusé. Délai non atteint ({$joursEcoules} j / {$delaiMinJours} requis). "
                        . "Prochain renouvellement le : {$prochaineDateDispo}.",
                        "Délai non atteint"
                    );
                    return back()->withInput();
                }
        ...
        // ── Délai OK (ou dérogation accordée) : enregistrement ───────────────
        ...
        try {
            ...
            if ($request->filled('certificat_hebergement_id')) {
                \App\Models\CertificatHebergement::where('id', $request->certificat_hebergement_id)
    ...
    // =========================================================================
    // RENOUVELLEMENTS BLOQUÉS — Rapport des tentatives journalisées
    // =========================================================================
    public function renouvellementsBloques(Request $request)
    {
        $this->authorize('dashboard.view');

        $query = DB::table('audit_logs')
            ->whereIn('audit_logs.action', ['RENOUVELLEMENT_BLOQUE', 'DEROGATION_RENOUVELLEMENT'])
            ->leftJoin('users', 'audit_logs.user_id', '=', 'users.id')
            ->select('audit_logs.*', DB::raw("CONCAT(users.prenom, ' ', users.nom) as agent_name"))
            ->orderByDesc('audit_logs.created_at');

        if ($request->filled('start_date')) {
            $query->whereDate('audit_logs.created_at', '>=', $request->start_date);
        }
        ...
        $total         = $query->count();
        $agentsUniques = (clone $query)->distinct()->count('audit_logs.user_id');
        $logs          = $query->paginate(25);

        return view('admin.demandes.renouvellements-bloques', compact('logs', 'total', 'agentsUniques'));
    }
    ...
    public function checkQuittance(Request $request)
    {
        ...
        return response()->json([
            ...
            'demande'   => [
                ...
                'photo'            => $demande->photo ? asset('app/' . $demande->photo) : asset('img/avatar.png'),
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            "nom_mere"                          => "required",
            "numero_passeport"                  => $request->sans_passeport ? "nullable" : "required",
            "date_emission_passeport"           => $request->sans_passeport ? "nullable|date" : "required|date",
            "date_expiration_passeport"         => $request->sans_passeport ? "nullable|date" : "required|date",
            "passeport_delivre_par"             => $request->sans_passeport ? "nullable" : "required",
            "etat_civil"                        => "required|string",
            "type_demande"                      => "required|string",
            "validite"                          => "required|string",
        ...
        $demandeExistante = Demande::where('numero_quittance', $request->numero_quittance)
            ->where('quittance_confirmee', false)->first();
        ...
        try {
            ...
            if ($request->numero_passeport != "") {
                ...
                // ── Sauvegarde dans impetrant_documents (fiche impétrant) ──────────
                $docExiste = \App\Models\ImpetrantDocument::where('impetrants_id', $impretrant->id)
                    ->where('numero_document', $request->numero_passeport)
                    ->exists();

                if (!$docExiste) {
                    \App\Models\ImpetrantDocument::create([
                        'impetrants_id'      => $impretrant->id,
                        'type_document'      => 'Passeport',
                        'numero_document'    => $request->numero_passeport,
                        'date_delivrance'    => $request->date_emission_passeport    ?: null,
                        'date_expiration'    => $request->date_expiration_passeport  ?: null,
...
if ($request->filled('certificat_hebergement_id')) {
    \App\Models\CertificatHebergement::where('id', $request->certificat_hebergement_id)
        ->update(['demande_id' => $demande->id]);
            ...
            if ($request->operation == "validate") {
                ...
                toastr()->success("Demande enregistrée avec succès");
                return redirect()->route('demandes.show', $demande->id)->with('just_created', true);
            } else {
                DB::rollBack();
    ...
    public function show(string $id)
    {
        ...
        $query = Impetrant::query()
            ->where('id', '!=', $demande->impetrants_id)
            ->with(['demandes' => function ($q) { $q->where('retire', 0)->orderByDesc('created_at'); }])
            ->where(function ($q) use ($imp) {
                $q->where(function($sub) use ($imp) {
                    $sub->where('nom', 'like', $imp->nom . '%')
                        ->where('prenom', 'like', substr($imp->prenom, 0, 3) . '%');
                });
        ...
        // Exclure les similarités déjà écartées
        ...
        $query->chunk(100, function ($others) use (&$similaires, $imp, $SEUIL, $excludedIds) {
            foreach ($others as $o) {
                ...
                if (!$autreDemande) continue;
                if (in_array($autreDemande->id, $excludedIds)) continue;

                // Même logique que similarities() — IdentitySimilarityService direct
                $result = IdentitySimilarityService::compare($imp, $o);
   ...
   $certificatHebergement = \App\Models\CertificatHebergement::with([
    'hebergeurCongolais', 'hebergeurEtranger', 'hebergeurSociete'
])->where('demande_id', $id)
  ->orWhere('heberge_impetrant_id', $demande->impetrants_id)
  ->first();
        return view("admin.demandes.show", [
            ...
            'certificatHebergement' => $certificatHebergement,
        ]);
    ...
    public function edit(string $id)
{
    ...
    return view("admin.demandes.newedit", compact("demande","pays","departements","etatsCivils","validites","pieces","employeurs","categories","usersActifs"));
}
    ...
    {
        $demandes = Demande::where('retire', 1)->orderBy('retire_le', 'desc')->paginate(20);
    ...
    public function takephoto($id){
        $demande = Demande::find($id);
        if ($demande == null) { toastr()->error("Impossible de traiter cette requête"); return back(); }
    ...
    {
        try {
            \Log::info("=== DÉBUT CHANGESTATE ===");
            \Log::info("ID: " . $id);
            \Log::info("Statut demandé: " . $request->statut_demande);

            $demande = Demande::with('impetrant')->findOrFail($id);
            \Log::info("Demande trouvée: " . $demande->id);

            if ($request->statut_demande === 'Approuvée') {
                \Log::info("Vérification contentieux...");

                $contentieuxActifs = Demande::where('impetrants_id', $demande->impetrants_id)
                    ->where('id', '!=', $demande->id)
                    ->where('statut_demande', 'Envoyée au contentieux')
                    ->with(['contentieux.motif', 'impetrant'])
                    ->get();

                \Log::info("Contentieux trouvés: " . $contentieuxActifs->count());

                if ($contentieuxActifs->count() > 0) {
                    $roleActuel     = auth()->user()?->role?->lib_role;
                    $rolesAutorisés = ['SuperAdmin', 'Admin'];

                    if (!in_array($roleActuel, $rolesAutorisés)) {
                        \Log::info("Bloqué par contentieux");
                        $details = $contentieuxActifs->map(function($d) {
                            $motifs = $d->contentieux->map(fn($c) => $c->motif?->lib_motif ?? 'Motif inconnu')->join(', ');
                            return "Dossier N°{$d->uuid} — Motif(s) : {$motifs}";
                        })->toArray();

                        return back()
                            ->with('contentieux_danger', true)
                            ->with('contentieux_details', $details)
                            ->with('contentieux_demandes', $contentieuxActifs->toArray());
                    }

                    toastr()->warning("Approbation forcée malgré un contentieux actif sur cet impétrant.");
                }
            }

            \Log::info("Avant vérification similarité...");

            if ($request->statut_demande === 'Approuvée') {
                $autresDemandes = Demande::with('impetrant')->where('id', '!=', $demande->id)->get();
                \Log::info("Demandes à comparer: " . $autresDemandes->count());

                foreach ($autresDemandes as $autreDemande) {
                    if (!$autreDemande->impetrant) continue;

                    $result = IdentitySimilarityService::compare($demande->impetrant, $autreDemande->impetrant);

                    if (in_array($result['decision'], ['CERTAIN', 'QUASI_CERTAIN'])) {
                        \Log::info("Bloqué par similarité");
                        toastr()->error("APPROBATION BLOQUÉE — Identité déjà existante (" . $result['score'] . "%)");
                        return back();
                    }
                }
            }
            ...
            $demande->save();
            \Log::info("Après save() - SUCCÈS");

            toastr()->success("Demande approuvée avec succès");
            return back();

        } catch (\Exception $e) {
            \Log::error("ERREUR CHANGESTATE: " . $e->getMessage());
    ...
    public function searchForContentieux(Request $request)
    {
        $q = trim($request->get('q', ''));
        if (strlen($q) < 2) return response()->json([]);

        $demandes = Demande::with(['impetrant', 'soitTransmis'])
            ->where(function($query) use ($q) {
                $query->where('uuid', $q)
                      ->orWhereHas('impetrant', function($qi) use ($q) {
                          $qi->where('nom', 'LIKE', "%$q%")->orWhere('prenom', 'LIKE', "%$q%");
                      });
            })
            ->whereNotIn('statut_demande', ['Envoyée au contentieux'])
            ->limit(10)->get()
            ->map(fn($d) => [
                'id'             => $d->id,
                'uuid'           => $d->uuid,
                'nom'            => $d->impetrant?->nom,
                'prenom'         => $d->impetrant?->prenom,
                'date_naissance' => $d->impetrant?->date_naissance
                    ? \Carbon\Carbon::parse($d->impetrant->date_naissance)->format('d/m/Y') : '-',
                'type_demande'   => $d->type_demande,
                'statut'         => $d->statut_demande,
                'soit_transmis'  => $d->soitTransmis?->numero,
            ]);

        return response()->json($demandes);
    }

    public function storeContentieuxGlobal(Request $request)
    {
        $request->validate([
            'demande_id'  => 'required|exists:demandes,id',
            'motifs_id'   => 'required|exists:motif_contentieuxes,id',
        ...
        try {
            ...
            $contentieux->demandes_id = $demande->id;
            ...
            toastr()->success("Dossier envoyé au contentieux avec succès");
            return back();
        } catch (\Exception $e) {
            DB::rollBack();
            toastr()->error($e->getMessage());
    ...
    public function storeremplirformation($id, Request $request){
        $demande = Demande::find($id);
        if ($demande == null) { toastr()->error("Impossible de traiter cette requête"); return back(); }
        if ($demande->statut_demande != "Approuvée") { toastr()->error("Cette demande n'est pas encore approuvée"); return back(); }

        try {
            $demande->numero_document  = $request->numero_document;
            $demande->date_emission    = $request->date_emission;
            $demande->date_expiration  = Carbon::parse($request->date_emission)->addYears((int)$demande->validite)->subDay();
    ...
    public function demandestats(){
        ...
        $demandes = collect([]);
        switch ($critere) {
            case "jour":    $demandes = collect(DB::select("select * from demandes dem join impetrants imp on dem.impetrants_id = imp.id where date(dem.date_demande)=date(curdate())")); break;
            case "semaine": $demandes = collect(DB::select("select * from demandes dem join impetrants imp on dem.impetrants_id = imp.id where week(dem.date_demande)=week(curdate()) and year(dem.date_demande)=year(curdate())")); break;
            case "mois":    $demandes = collect(DB::select("select * from demandes dem join impetrants imp on dem.impetrants_id = imp.id where month(dem.date_demande)=month(curdate()) and year(dem.date_demande)=year(curdate())")); break;
            case "annee":   $demandes = collect(DB::select("select * from demandes dem join impetrants imp on dem.impetrants_id = imp.id where year(dem.date_demande)=year(curdate())")); break;
        }
    ...
    public function demandesattribuees(){
        ...
        $demandes = collect([]);
        switch ($critere) {
            case "jour":    $demandes = collect(DB::select("select * from demandes dem join impetrants imp on dem.impetrants_id = imp.id where date(dem.date_attribution)=date(curdate()) and attribue=1")); break;
            case "semaine": $demandes = collect(DB::select("select * from demandes dem join impetrants imp on dem.impetrants_id = imp.id where week(dem.date_attribution)=week(curdate()) and year(dem.date_attribution)=year(curdate()) and attribue=1")); break;
            case "mois":    $demandes = collect(DB::select("select * from demandes dem join impetrants imp on dem.impetrants_id = imp.id where month(dem.date_attribution)=month(curdate()) and year(dem.date_attribution)=year(curdate()) and attribue=1")); break;
            case "annee":   $demandes = collect(DB::select("select * from demandes dem join impetrants imp on dem.impetrants_id = imp.id where year(dem.date_attribution)=year(curdate()) and attribue=1")); break;
        }
    ...
    public function similarities($id)
    {
        ...
        $query = \App\Models\Impetrant::query()
            ...
            ->where(function ($q) use ($imp) {
                $q->where(function($sub) use ($imp) {
                    $sub->where('nom', 'like', $imp->nom . '%')->where('prenom', 'like', $imp->prenom . '%');
                });
                if (!empty($imp->date_naissance)) {
                    $q->orWhere(function($sub) use ($imp) {
                        $sub->where('nom', 'like', $imp->nom . '%')->where('date_naissance', $imp->date_naissance);
                    });
                }
            });
        ...
        $query->chunk(100, function ($impetrants) use (&$sims, $imp, $SEUIL_AFFICHAGE, $demande) {
            foreach ($impetrants as $potentialMatch) {
                $result = \App\TechnoDev\src\Classes\IdentitySimilarityService::compare($imp, $potentialMatch);
                $score  = $result['score'] ?? 0;
                if ($score < $SEUIL_AFFICHAGE) continue;
                ...
                $isRejected = \App\Models\SimilarityRejection::where(function ($q) use ($demande, $autreDemande) {
                    $q->where('demande_base_id', $demande->id)->where('demande_similaire_id', $autreDemande->id);
                })->orWhere(function ($q) use ($demande, $autreDemande) {
                    $q->where('demande_base_id', $autreDemande->id)->where('demande_similaire_id', $demande->id);
                })->exists();
        ...
        $sims = collect($sims)->sortBy('score')->values();
    ...
    public function storePhoto(Request $request, $id){
        $demande = Demande::find($id);
        if ($demande == null) {
            return response()->json(["code" => "180", "message" => "La demande à associer avec la photo n'est pas reconnue"]);
        }
        ...
        try {
            ...
            if (file_put_contents($path, base64_decode($photo))) {
                ...
                $demande->save();
                return response()->json(["code" => "200", "message" => "Photo prise avec succès"]);
            } else {
                return response()->json(["code" => "187", "message" => "Un problème est survenu lors de la prise de la photo"]);
            }
        } catch (Exception $e) {
            return response()->json(["code" => "187", "message" => $e->getMessage()]);
        }
    }

    public function exportToJson(){
        $results = DB::select("
            SELECT impetrants.nom, impetrants.prenom, impetrants.sexe,
                ...
                pays.lib_pays AS nationalite, demandes.photo, demandes.id AS demande_id
            FROM impetrants
            ...
            WHERE demandes.export_json != 'exporte' AND statut_demande = 'Approuvée'
        ");
        ...
        if ($results != null) {
            ...
            foreach ($results as $result) {
                ...
                if (File::exists($photoPath)) {
                    File::copy($photoPath, $exportFolderPath . '/' . basename($result->photo));
                    $exportData[] = [
                        'nom'            => $result->nom,
            ...
            file_put_contents($exportFolderPath . '/exported_data.json', json_encode($exportData, JSON_PRETTY_PRINT));

            $zipFilePath = storage_path('app/public/' . $exportFolderName . '.zip');
            $zip = new ZipArchive();
            $zip->open($zipFilePath, ZipArchive::CREATE);
            foreach (File::allFiles($exportFolderPath) as $file) {
                $zip->addFile($file->getPathname(), basename($file));
            }
            $zip->close();

            Demande::whereIn('id', collect($results)->pluck('demande_id')->toArray())->update(['export_json' => 'exporte']);
    ...
    public function downloadJson(){
        $demandes = Demande::select("type_demande","impetrants_id","uuid","avenue_rue","numero_adresse","quartiers_id")
            ->with(["impetrant:id,nom,prenom,sexe,date_naissance,lieu_naissance,nationalites_id","impetrant.pays:id,lib_pays","quartier:id,lib_quartier"])
            ->where("tag_demande","IMPRESSION")->where("statut_demande","Approuvée")
            ->where("attribue","0")->where("type_demande","Carte de résident temporaire")->get();
    }
    ...
    public function procheExpiration(){
        ...
        $demandes = Impetrant::whereHas("demandes", function($query) use ($dateDans3Mois) {
            $query->where("date_expiration", "<=", $dateDans3Mois)->where("date_expiration", ">=", now());
    ...
    public function compareSimilarity(){
        $base    = Demande::find(request()->get("base"));
        $similar = Demande::find(request()->get("similar"));
        if ($base == null || $similar == null) { toastr()->error("Impossible de traiter cette requête"); return back(); }
    ...
    {
        $demande = Demande::with('impetrant')->find($id);
        if (!$demande) { toastr()->error("Demande introuvable"); return back(); }

        DB::beginTransaction();
        try {
            $impetrant = $demande->impetrant;
            $demande->delete();
            if ($impetrant && $impetrant->demandes()->count() === 0) { $impetrant->delete(); }
            DB::commit();
    ...
    {
        $request->validate(['type' => 'required|string', 'numero' => 'required|string']);
        $results = collect();

        switch ($request->type) {
            case 'PASSEPORT':
                $results = Demande::whereHas('documents', function ($q) use ($request) {
                    $q->where('type_document', 'Passeport')->where('numero_document', $request->numero);
                })->with('impetrant')->get();
                break;
            case 'NUM_FICHE':
                $results = Demande::where('uuid', $request->numero)->with('impetrant')->get();
                break;
            case 'VISA':
            case 'CRT':
                $results = Demande::where('numero_document', $request->numero)->with('impetrant')->get();
                break;
        }

        return response()->json(['found' => $results->isNotEmpty(), 'results' => $results]);
    }
}